...
`]);
    ...
    assert.equal(modernSeries.totals.totalTokens, 1130, "主会话与辅助模型只计一次，推理不重复计入输出");
    assert.equal(modernSeries.daily[0].totalCost, 2.2, "历史日独立保留估算成本");
    assert.equal(modernSeries.daily[1].totalCost, 6.5, "同一天按session先选实际或估算，再求和");
    assert.equal(modernSeries.totals.totalCost, 8.7, "跨日费用与逐session选择一致");
    assert.equal(modernBreakdown.totals.totalCost, 8.7);
    ...
    assert.equal(modernBreakdown.modelDaily.reduce((sum, row) => sum + row.tokens, 0), 1130, "日趋势与包含辅助调用的总计一致");
    ...
    assert.equal(partial.totals.totalTokens, 1130, "失败profile不能拖垮健康profile");
