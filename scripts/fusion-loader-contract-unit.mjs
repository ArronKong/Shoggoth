...
const checks = [
  ...
  ["shared track rule runs both sizes at 1.5x speed with an infinite linear cycle", /animation-duration:\s*calc\(1\.6s\s*\/\s*1\.5\)/.test(trackRule) && /animation-timing-function:\s*linear/.test(trackRule) && /animation-iteration-count:\s*infinite/.test(trackRule)],
  ["second ball stays exactly half a cycle ahead at 1.5x speed", blocksAfter(styles, ".rightTrack").some((block) => /animation-delay:\s*calc\(-0\.8s\s*\/\s*1\.5\)/.test(block))],
