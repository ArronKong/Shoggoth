...
- Antigravity 默认模型执行不再被额外的模型目录和登录预检阻塞，实际认证仍由 CLI 完成。
- 区分原生启动等待与消息接收等待，明确未发送的启动失败不会再显示为执行状态未知。
- 将 CLI 明确拒发的网络、地区及账号资格错误传递到聊天界面；保留未知轮次的防重复执行保护。

## 0.8.127 — 2026-09-23

- 修复正式环境加密数据兼容：Developer ID 包可读取旧本地密文并保留既有钥匙串身份。
- 内部测试渠道与签名分离，支持保留 Developer ID 签名且不公证、不启用自动更新。
- 加密数据仍锁定时，服务卡片不再显示“服务运行正常”。

## 0.8.126 — 2026-09-23

- 修复 Grok 本地拒发后会话无法继续、超时请求迟发及附件幂等校验问题；已发送但结果未知的请求继续防止重复执行。
- 改善 Antigravity 原生审批的渲染等待、取消、超时和迟到响应处理；不自动放宽工具权限。
- 为 Pi、DeepSeek Harness 和 Antigravity 增加按账号、配置与认证身份隔离的模型目录缓存。
- 保存加密执行记录，在 Service 重启后核验原生任务结果；具体连接、模型、协议与审批错误保留到聊天界面。
- Claude 接入方案保持不变；此版本首先作为内部测试包验收。
