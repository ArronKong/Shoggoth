...
const { CLAUDE_CODE_RUNTIME } = require("./claude-code-runtime-paths");
...
function validateTurn(value) {
  return exact(value, [
    "id", "operationId", "fingerprint", "userMessageUuid", "acceptance", "status",
    "errorCode", "assistantMessages", "responseId", "createdAt", "updatedAt",
  ])
    && safeString(value.id, 512) && safeString(value.operationId, 512)
    && /^[a-f0-9]{64}$/u.test(value.fingerprint)
    && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu
      .test(value.userMessageUuid)
...
function validateSession(value) {
  return exact(value, [
    "id", "remoteSessionId", "source", "cwd", "title", "archived",
    "createdAt", "updatedAt", "turns",
  ])
    && safeString(value.id, 512) && safeString(value.remoteSessionId, 512)
    && value.remoteSessionId === value.id
...
function validateLedger(value, runtimeProfileId, workspaceShardId) {
  if (!exact(value, ["schemaVersion", "runtime", "runtimeProfileId", "workspaceShardId", "sessions"])
    || value.schemaVersion !== SCHEMA_VERSION || value.runtime !== CLAUDE_CODE_RUNTIME
    ...
    throw ledgerError("CLAUDE_CODE_LEDGER_INVALID", "Claude Code runtime ledger is invalid");
  }
  ...
  for (const session of value.sessions) {
    if (sessionIds.has(session.id) || sources.has(session.source)) {
      throw ledgerError("CLAUDE_CODE_LEDGER_INVALID", "Claude Code ledger contains duplicate sessions");
    }
    ...
    for (const turn of session.turns) {
      if (turnIds.has(turn.id) || operations.has(turn.operationId)
        || userMessages.has(turn.userMessageUuid)) {
        throw ledgerError("CLAUDE_CODE_LEDGER_INVALID", "Claude Code ledger contains duplicate turns");
      }
      turnIds.add(turn.id);
      operations.add(turn.operationId);
      userMessages.add(turn.userMessageUuid);
    }
  }
  return value;
}

class ClaudeCodeRuntimeLedger {
  constructor(options = {}) {
    this.fs = options.fs || fs;
    if (!validRuntimeProfileId(options.runtimeProfileId)) {
      throw ledgerError("CLAUDE_CODE_LEDGER_OPTIONS_INVALID", "Claude Code runtime profile id is invalid");
    }
    ...
    if (typeof options.stateRoot !== "string" || !path.isAbsolute(options.stateRoot)
      ...
      throw ledgerError("CLAUDE_CODE_LEDGER_OPTIONS_INVALID", "Claude Code ledger paths are invalid");
    }
    ...
    if (relative !== path.join(this.runtimeProfileId, this.workspaceShardId)
      ...
      throw ledgerError("CLAUDE_CODE_LEDGER_OPTIONS_INVALID", "Claude Code ledger escapes its root");
    }
    ...
    if (stateRelative === "" || stateRelative === ".." || stateRelative.startsWith(`..${path.sep}`)
      ...
      throw ledgerError("CLAUDE_CODE_LEDGER_OPTIONS_INVALID", "Claude Code state root is untrusted");
    }
  ...
  open() {
    ...
    try {
      ...
      throw ledgerError("CLAUDE_CODE_LEDGER_INVALID", "Claude Code ledger is malformed");
    }
  ...
  snapshot() {
    if (!this.data) throw ledgerError("CLAUDE_CODE_LEDGER_CLOSED", "Claude Code ledger is closed");
    return structuredClone(this.data);
  }

  update(mutator) {
    if (!this.data || typeof mutator !== "function") {
      throw ledgerError("CLAUDE_CODE_LEDGER_CLOSED", "Claude Code ledger is closed");
    }
  ...
  _write(value) {
    ...
    if (Buffer.byteLength(serialized, "utf8") > MAX_LEDGER_BYTES) {
      throw ledgerError("CLAUDE_CODE_LEDGER_TOO_LARGE", "Claude Code ledger exceeds its limit");
    }
