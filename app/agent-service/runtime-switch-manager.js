...
class RuntimeSwitchManager {
  constructor(options = {}) {
    for (const [value, methods, label] of [
      [options.productStore, ["getAgentProfile", "putAgentProfile", "listWorkRuns"], "ProductStore"],
  ...
  async switchProfile(input = {}) {
    ...
    let legacyAccount;
    try {
      legacyAccount = runtimeAccountForLegacyProfile(profile);
    } catch {
      throw switchError(
        "RUNTIME_SWITCH_ACCOUNT_UNSUPPORTED",
        "Runtime switch 暂不支持此 RuntimeAccount binding",
      );
    }
    if (profile.runtimeAccountId !== legacyAccount.runtimeAccountId) {
      throw switchError(
        "RUNTIME_SWITCH_ACCOUNT_UNSUPPORTED",
        "Runtime switch 暂不支持显式 RuntimeAccount binding",
      );
    }
    let candidateAccount;
    try {
      candidateAccount = runtimeAccountForLegacyProfile({
        ...profile,
        runtime: candidateBinding.runtime,
        runtimeProfileId: candidateBinding.runtimeProfileId,
      });
    } catch {
      throw switchError(
        "RUNTIME_SWITCH_ACCOUNT_UNSUPPORTED",
        "candidate RuntimeAccount binding 无法安全推导",
      );
    }
    if (candidateAccount.runtimeAccountId !== candidateBinding.runtimeAccountId) {
      throw switchError(
        "RUNTIME_SWITCH_ACCOUNT_UNSUPPORTED",
        "candidate RuntimeAccount binding 无法安全推导",
      );
    ...
    try {
      ...
      this.productStore.putAgentProfile({
        ...current,
        runtime: candidateBinding.runtime,
        runtimeProfileId: candidateBinding.runtimeProfileId,
      });
      ...
      try {
        ...
        if (current && sameBinding(profileBinding(current), candidateBinding)) {
          this.productStore.putAgentProfile({
            ...current,
            runtime: previousBinding.runtime,
            runtimeProfileId: previousBinding.runtimeProfileId,
          });
        }
  ...
  async recover() {
    ...
    if (sameBinding(current, operation.candidateBinding)) {
      ...
      this.productStore.putAgentProfile({
        ...profile,
        runtime: operation.previousBinding.runtime,
        runtimeProfileId: operation.previousBinding.runtimeProfileId,
      });
