...
class ClaudeCodeRuntimeHandle {
  constructor(binding, host) {
    this.runtime = CLAUDE_CODE_RUNTIME;
...
class ClaudeCodeRuntimeAdapter {
  constructor(options = {}) {
    if (!options.runtimePool || typeof options.runtimePool.get !== "function"
      ...
      throw adapterError(
        ...
        "Claude Code adapter requires a ClaudeCodeRuntimePool",
      );
    }
  ...
  async acquire(value, options = {}) {
    ...
    if (binding.runtime !== CLAUDE_CODE_RUNTIME) {
      throw adapterError(
        "RUNTIME_UNSUPPORTED",
        `Claude Code adapter cannot run ${binding.runtime}`,
      );
    }
    const host = await this.runtimePool.get(binding, options);
    const existing = this.handles.get(host);
    if (existing) return existing.handle;
    const handle = Object.freeze(new ClaudeCodeRuntimeHandle(binding, host));
  ...
  stop(value) {
    ...
    if (typeof value === "string") id = value;
      ...
      if (binding.runtime !== CLAUDE_CODE_RUNTIME) {
        throw adapterError(
          "RUNTIME_UNSUPPORTED",
          `Claude Code adapter cannot stop ${binding.runtime}`,
        );
      }
