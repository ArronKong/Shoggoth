...
class PiRuntimeAdapter {
  ...
  async acquire(value, options = {}) {
    ...
    if (binding.runtime !== PI_RUNTIME) {
      throw adapterError("RUNTIME_UNSUPPORTED", `Pi adapter cannot run ${binding.runtime}`);
    }
    const host = await this.runtimePool.get(binding, options);
    const existing = this.handles.get(host);
    if (existing) return existing.handle;
    const handle = Object.freeze(new PiRuntimeHandle(binding, host));
  ...
  stop(value) {
    ...
    if (typeof value === "string") id = value;
      ...
      const binding = runtimeBinding(value);
      if (binding.runtime !== PI_RUNTIME) {
        throw adapterError("RUNTIME_UNSUPPORTED", `Pi adapter cannot stop ${binding.runtime}`);
      }
...
module.exports = { PI_CAPABILITIES, PiRuntimeAdapter };
