...
const GROK_BUILD_CAPABILITIES = runtimeCapabilities({
  "session.start": true,
  "session.resume": true,
  "session.read": true,
  "session.list": true,
...
class GrokBuildRuntimeAdapter {
  constructor(options = {}) {
    if (!options.runtimePool || typeof options.runtimePool.get !== "function"
      ...
      throw adapterError(
        ...
        "Grok Build adapter requires a GrokBuildRuntimePool",
      );
    }
  ...
  async acquire(value, options = {}) {
    ...
    if (binding.runtime !== GROK_BUILD_RUNTIME) {
      throw adapterError(
        "RUNTIME_UNSUPPORTED",
        `Grok Build adapter cannot run ${binding.runtime}`,
      );
    }
    const host = await this.runtimePool.get(binding, options);
    const existing = this.handles.get(host);
    if (existing) return existing.handle;
    const handle = Object.freeze(new GrokBuildRuntimeHandle(binding, host));
  ...
  stop(value) {
    ...
    if (typeof value === "string") id = value;
      ...
      if (binding.runtime !== GROK_BUILD_RUNTIME) {
        throw adapterError(
          "RUNTIME_UNSUPPORTED",
          `Grok Build adapter cannot stop ${binding.runtime}`,
        );
      }
