        ...
        {error ? <div className="settings-inline-state" role="alert"><div><strong>{t("settings.shoggothUnavailable")}</strong><p>{t("settings.serviceRetryHint")}</p></div><button className="ui-cbtn ui-cbtn--sm" onClick={onRetry}>{t("settings.retry")}</button></div>
          : !status ? <p className="ui-hint">{t("common.loading")}</p> : <>
            <div className="settings-service-top">
              <div className={`settings-service-icon ${status.service.healthy ? "is-healthy" : ""}`} aria-hidden="true">
              ...
              <div className="settings-service-copy"><h4>{t(status.service.healthy ? "settings.serviceRunning" : "settings.serviceStopped")}</h4><p>{t("settings.servicePurpose")}</p></div>
            ...
            <dl className="settings-service-facts">
              ...
              <div><dt>{t("settings.serviceData")}</dt><dd>{t(status.service.pendingCommandsLocked || status.service.mcpCredentialsLocked ? "settings.serviceLocked" : "settings.available")}</dd></div>
            </dl>
            {(status.service.pendingCommandsLocked || status.service.mcpCredentialsLocked) && <p className="ui-hint status-error">{t("settings.shoggothLocked")}</p>}
