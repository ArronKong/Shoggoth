// Adapted from ybouane/liquidglass (MIT); the glass fragment shader has local changes.
// Source revision, attribution and full terms: resources/legal/licenses/source/LIQUIDGLASS.txt
// GLSL (WebGL1 / GLSL ES 1.0) shader sources for the liquid-glass effect.
// Ported and adapted from ybouane/liquidglass (MIT) — the reference WebGL implementation
// identified by the deep-research pass as the way to exceed the SVG/backdrop-filter
// ceiling (refracts the scene with a real biconvex bevel + Fresnel + multi-light
// specular). Pipeline: blit (upload) → 9-tap Gaussian blur (H+V × N) → glass composite.
...
float bevelHeight(float d, float zR) {
  if (d <= 0.0) return 0.0;
  if (d >= zR) return zR;
  // Convex squircle (the profile Apple favours): a softer flat<->curve transition than a
  // plain circle, so refraction gradients stay smooth on stretched rounded-rects instead
  // of producing a harsh interior edge. d/zR in [0,1] from rim to bezel end.
  float u = 1.0 - d / zR;
  return zR * pow(1.0 - u * u * u * u, 0.25);
}
...
void main() {
  ...
  // 轮廓抗锯齿：±AA（设备像素）的覆盖率羽化带对称跨在真边界两侧。旧实现在
  // sdf>0 就走阴影分支 return，把 mask 的羽化带在正边界处拦腰截断——玻璃以
  // α≈0.16 硬跳变收尾，1x 屏上肉眼可见锯齿。带内不再叠外阴影（<1px 差异不可见）。
  const float AA = 0.75;
  ...
  vec2 refrPx;
  if (u_bevelMode < 0.5) {
    // Biconvex EDGE refraction: hGrad is ~0 in the flat centre and steep in the bevel
    // band, so Refraction bends only the edges while the centre stays clear (1:1).
    // The old uniform center-magnification term is removed — it made Refraction read as
    // "zoom the whole inside" instead of edge distortion.
    vec2 exitRefr = hGrad * refrPow;
    vec2 entryRefr = hGrad * refrPow;
    vec2 throughRefr = entryRefr * thickNorm * 0.5;
    refrPx = (exitRefr + entryRefr + throughRefr) * u_refract * 42.0;
  } else {
